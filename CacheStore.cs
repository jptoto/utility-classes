...
{
    /// <summary>
    /// Lightweight caching of objects
    /// Useage
    ///    
    /// if (!CacheStore.Exists<string>(service.Name))
    //{
    //    // This is a newly offline service, it's not in the cache so add it and send an SMS
    //    private CacheStore.Add<string> 
    //(
    ...
    public static class CacheStore
    {
        ...
        public static void Add<T>(string key, T value)
        {
            ...
            if (value.GetType() != type)
                throw new ApplicationException(
                    String.Format(
                        "The type of value passed to cache {0} does not match the cache type {1} for key {2}",
        ...
        public static void Remove<T>()
        {
            ...
            {
                if (_cache.ContainsKey(type.Name) == false)
                    throw new ApplicationException(String.Format("An object of type '{0}' does not exists in cache",
                                                                 type.Name));
                lock (_sync)
                {
